...
| Translation language — where is it chosen, and who pays? | Build-time, per book: `--lang` (default `english`), one language per book. The cost is borne by whoever runs the build, not the maintainer — a reader who wants another language runs biread themselves with their own key. English content is byte-identical; cache is per-language (`translations.<code>.json`). |
