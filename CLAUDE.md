...
| A second library that lists books it has not made? | **Read the cover, and offer only what exists.** Standard Ebooks lists what is in the public domain beside what it has actually produced, in identical markup, so the row above was offering readers three George Sand novels that carry no text at all: the page behind each says *We don't have this ebook in our catalog yet*, and its `/text/single-page` is a 404. Picked on the lookup screen, the build then failed at fetch time minutes later, which is the standing rule exactly inverted — fail loudly and early, never offer what cannot be delivered. The tell is in the search markup and costs no second request: a produced book's row draws the cover the site made for it, an unmade one draws `<div class="placeholder-cover">` in its place. Measured complementary over **173 rows** across eight searches, grid view and list — every row has exactly one of the two — and agreeing with what the text URL answered on all **eleven** paths probed. It is the **cover** that is read, not the placeholder, because the two failures are not equal: were the placeholder renamed, reading it would call every unmade book produced and put the silent fault straight back, where reading the cover fails the other way and the second library merely goes quiet, which the card already says plainly. A HEAD per result was declined for multiplying requests, and probing is not free anyway — an ebook page carries a honeypot link that bans the caller's IP for a day. Live: George Sand 5 listed and 2 offered, Zola 8 and 2, Flaubert 4 and 3. **And the same function was losing a book in the other direction.** Two translators are joined in a path by an underscore, and the character was missing from the pattern entirely, so the Devil's Pool — produced, 17 chapters, 674 paragraphs, and one of only *two* George Sand editions that exist there — was dropped from every search it appeared in. It is credited the way the site credits it, *Jane Minot Sedgwick and Ellery Sedgwick*. |
| Should a card name the books the second library has not made? | **Not yet decided; nothing is claimed in the meantime.** They are simply absent, and where an author's whole shelf there is unmade the card falls to its existing sentence, *Standard Ebooks has none by X either* — true about what can be read and a slight overclaim about what is listed. The alternative is to name them and say they cannot be had, which is more informative and puts three dead titles on a card whose whole difficulty is that a reader must judge a pairing we cannot vouch for. `Book.produced` carries the fact either way, so this is a copy decision and not a build one. |
...
| An empty half of the front door? | **A page of the book it is offering, where step two has its sample.** Step one asked its questions in a column and left the rest of the screen bare, so at 1800px the content sat with 310px on its left and 630px on its right and read as a page pushed aside. Every width fix made it worse somewhere else: matching the panels to the type moved the imbalance from inside the block to around it, matching the container to the panels gave the shelf back its 160px jump, and narrowing everything to one measure cost the shelf its card size, which is measured elsewhere in this file at 277px and a truncated lead. The screen was not too wide; it was **missing a column**. What goes in it is the answer to the question a reader is actually asking at the door, and it is the one thing the builder never showed before the price screen: a finished spread, at the book's own 7:5, set in the reader's own face, with folio numbers and a page's worth of type **overrunning at the foot** the way a page does rather than two paragraphs floating in a box. The text is real and comes from the Candide on the shelf, so the two columns correspond because they were aligned and not because they were written to fit. It stands down on the shelf route, which wants the whole column for its cards, and below 1260px, which is exactly where the container stops growing: any narrower and the tab strip would shrink on the two routes that carry the page and not on the one that does not. Lifted 58px clear of the row it sits in, so its label stands level with the **headline** it answers rather than with the question below it: level with the masthead was tried and is too high, since the page then reads as a second thing the screen opens with instead of the answer to what the screen is asking. And what the panel says under it names the gesture rather than the property, since *the French hoverable phrase by phrase* is an adjective doing an adverb's work: **any phrase in the French translated under the pointer**. Two faults found by using it. In the flow it hung off the step, so the first press — which docks the headline — moved it **122px up the page and left it there**, a page of a book leaping the height of a paragraph for a reason that has nothing to do with it; it is out of the flow now and holds one place in every state, which also settles the lift argument above, since the position no longer depends on how tall the question is. And it was set like a column of newsprint rather than a page of a book: type to the edges, lines close, one paragraph ending where the next begins. It has a book's margins, a book's leading and an indent that can be seen, which costs it four lines a side and buys the only thing the panel is for. It is also **headed the way the reader heads its own pages** — the language tag in the outer top corner and the folio in the outer foot — where it had been carrying the centred title over a rule that belongs to the builder's *sample* spread on step two: a panel showing what you end up with must show what you end up with, and the tag is sized to the miniature rather than copied at the reader's 9px, which on a page a quarter the width would read as a caption. Its height was then found by measuring rather than by taste: four positions rendered, and the one whose **centre falls within 10px of the column's beside it** is the one that looks settled, which also puts its label level with *Step one of two* and the head of the page level with the question. |
| Which is louder, the thing you press or the thing you look at? | **The thing you press, and it is measured rather than judged.** Salience is area, contrast against the paper and elevation, and the page of a book on the door beat the dropzones on all three: **145,900px² against 80,500**, **1.135 against 1.095**, and a shadow against none. So the brightest, largest, most raised object on the screen was the one thing nobody can press, while the only strong element left was a **disabled** Next. The dropzones stand off the paper now (`--lifted`, `--lift`, per theme) and read as slots rather than as captions, which takes them to **1.139** and puts them first. What was **not** done matters as much: the panel is untouched. The dropzones were the palest thing on this screen before the panel existed and would be after it, so quieting the illustration would have fixed the ordering and left the absolute fault where it was. Three alternatives were drawn on the live page and measured beside this one, and are kept because the numbers say something. **Quieting the panel** (no shadow, page at 50% over the paper) takes it to 1.066 and wins by making its neighbour dimmer, which is a relative fix to an absolute problem; it is one line of CSS if the panel still pulls too hard. **Shrinking the panel** to 372px is the weakest of the four and the most instructive: it costs two lines of type a side and the panel *still* outranks the dropzones, because area was never the leading term. **Both together** is what shipped, in its lightest form: the panel gives up its shadow, which is the strongest of the three cues at this distance and costs nothing in legibility, and one step of tone (`--page-quiet`, per theme), which takes it to **1.118** against the slots' 1.139 — a real gap where 1.135 against 1.139 was a tie. Full quieting to 1.066 was drawn and declined: at that weight the page reads as a watermark, and the panel is there to be read. The night theme keeps one inversion nothing here can fix: the reader's page is light in both themes, so on a dark ground it is the brightest thing on the screen by construction. |
...
| What does a card say about the hover? | **What the reader's own header calls it, as a state and not a quantity.** `French + Phalen · hover translations · EPUB + PDF` said three things in one middot run and two of them belonged elsewhere. The **edition** is named on the credit line two lines above and in the sentence above the shelf, so saying it a third time, as an equation, on a page that speaks in phrases everywhere else, was the last of that dialect: gone. The **formats** are a fact about the download wearing the dress of a fact about the book, so they moved to the drawer, beside the weight, which is the same kind of fact. What is left is the hover, and it was a bare noun that does not say whether the book has it or is merely about it: the reader's header settled on **Hover to translate** on the grounds that one product must not be spoken of in two vocabularies, and the shelf was the last place still calling it something else. So a card says `Hover to translate: included` or `not made yet`, and **part-glossed is not a third state** — it is a quantity, and in a slot whose other values are states it reads as one that does not exist. A book with its opening bought says *not made yet*, which is true of the book and **under-claims**, the safe direction: a reader who hovers the first page and finds a gloss has been given something rather than misled. The count goes to the drawer, where the quantities are. The hoisting above the shelf is now **fact by fact** rather than line by line, since two books can agree on what is on their pages and differ about the hover; on today's shelf that leaves six cards saying nothing at all under the action line and one saying the two things that are its own. Refines the row above, which is otherwise unchanged. *(Superseded on the hover itself: see the row below. The fact-by-fact hoisting and the formats' move to the drawer stand.)* |
| Is the hover a state a card may claim at all? | **No, because every book here has it, and the card was saying otherwise.** `Hover to translate: included` stood on Micromégas and nowhere else, which reads as a capability the other six lack — and it is false: an unglossed book is not a book without hover, since the reader's own header finishes one as it is read, a page ahead, on the reader's key. So the shelf was telling a reader six of its seven books could not do a thing all seven do. Both available states were wrong in the same direction: `included` claims exclusivity the book does not have, `not made yet` reads as an absence rather than as work not yet paid for. The face carries **nothing** about the hover now, the sentence above the shelf drops its clause, and Micromégas is left saying the one thing that is genuinely its own — that it carries a translation of ours beside a published one. What is real about the difference is *how much is already made*, which is a quantity and goes where the quantities are: the drawer says **hover translations throughout** for a whole book and *on the first 126 passages* for a part one, and says nothing at all where none are made, because counting to zero is the same false claim in arithmetic. Found by a reader asking why one card had a status and its neighbour did not, which the previous row had answered correctly and about the wrong question: the cards were consistent with each other and inconsistent with the product. |
| What is left on the credit line? | **The edition, and nothing about it.** `English: Towle · 1873 · abridged · 37 chapters` named the translation and then judged it. **Abridged** was moved onto this line when the pills row came down, on the reasoning that abridgement is a fact about *this* English — true, and it still reads as a defect stamp, now standing where a translator is named and in the same voice as the year. **37 chapters** is a fact about how a book is divided, which is not something anybody chooses a novel on, and the number alone tells a reader nothing: 37 against 37 and 47 against 46 print identically here. Neither fact is lost. Where the two editions divide the book differently the book's **own note** says so in a sentence, under the picker, where a build is being chosen and it can change something; where a translation cuts, what a reader actually meets is the coverage line already on the card. Refines the two rows above, which are otherwise unchanged. |
| Which of the two things a card does is the one it offers? | **Reading the finished book, and the card says nothing else.** A card with a file behind it ended `Ready to read ↓` and then, directly under it, an underlined, arrowed `Or build it yourself · about 3 min →`: two arrows, two targets, on a surface that is itself one button, and the second of them was the loudest line on the card. The shelf exists so that a book can be picked up finished; building the same book again is the rare path and was reading as half the choice. Two changes. The action line names what a reader does — **Read it now** — with the same going arrow every other card carries, because a downloading arrow is what a browser's own controls use and it made the one card handing over a real book read as the technical option. And building your own moves into the **drawer**, under the pointer, beside the weight and the formats: one gesture for anyone looking for it, invisible to everyone else, and the minutes go with it, which is where the rest of the quantities already are. The cost is the drawer's own standing limit, and it is a real one: a touchscreen cannot open it, so a phone reader can take any of these books and can no longer build one from the shelf — the lookup screen and the other two routes are untouched. Cards come down a line each. **And then the card did not do it**, which is the half of this row that was written and not built: `takeBook` clicked an `<a download>`, so the line renamed *Read it now* went on fetching a file, and the copy above was describing an intention rather than the code. Found by a reader pressing it, twice, after the done screen had been fixed for exactly the same fault — this is the press that needs no build and no key, so it is the one anybody meets first, and its still fetching a file made the other fix look as though it had not worked. A published book is a real address on this site, so its tab is simply sent there, which also makes it **the only book here that can be bookmarked and reloaded**: one built in the browser lives in the tab it was written into. Taking the file away moves into the drawer, above building your own, beside the weight and the formats it is a fact about. Verified against the deployed page rather than a stub, since the fault was never in the copy. |
| Why is the page on the front door two thirds full? | **Because its type was a figure in a stylesheet, and one figure cannot be right in two engines.** The panel is a page of a finished book, so it is filled the way the reader fills one: whole lines to the foot, the paragraph it cannot finish broken. It was set at 8.4px because that is what fits **WebKit**, which sets this column three lines longer than Chromium does at the same size, so Chromium drew 14 lines where WebKit draws 17 and left **48px** of empty paper under the last line: a page of a book, in the panel that answers *what do I end up with*, ending a third of an inch early. The size is now measured at load (`fitTaste`) — the largest at which the last paragraph's own foot still stands inside the page — which lands on **8.9px in Chromium and 8.4px in WebKit, 2px of slack in both**, and needs no engine calibration to stay right. Two things it inherits from elsewhere in this file. It is fitted **after asking for Charis SIL by name**, because fitting before the face arrives fits to the fallback, which is exactly how the EPUB lost the last lines of its pages. And the CSS figure stays as the floor, so a page with no script still reads. What was **not** done: growing the type further, which this text cannot pay for — 9.0px overruns Chromium by a pixel, and anything larger has to come off the page as words. **The test passed throughout**, because it measured `scrollHeight`, which is clamped to the box it is measured in and reports a two-thirds-full page as exactly full; it reads the last paragraph's own rectangle now. |
| How much is there to read on the front door? | **Four blocks of grey, not six, and nothing that is left was shortened.** The complaint was noise, and the first cut answered it as a question about *space*: every note trimmed to the clause the screen did not already carry. That was wrong twice over. The notes became fragments — *Every word stays your translator's* under a tab reading *Align an edition I own* — obvious to whoever wrote them and unobvious to a reader; and the real weight is not the height of the prose but **how many separate things a reader has to read and digest** around one question. Step one carried six: the line under the headline (13 words), a note under the tabs (25), the file hint (9), the line by the button (15), the note under the page (25), and a strip at the foot (14). 101 words to answer *which book, and by which route*. Two go **whole**, and they are the two that were saying something already said: the note under the tabs, which opened by restating the tab that had just been pressed, and the assure strip, which repeated the promise standing 30px above it. The note under the page then loses only its opening — *One file, both pages* is the line under the headline again, two hundred pixels down — and keeps the two facts a still picture cannot show: where the finished book opens, and the hover. **56 words in four blocks**, every sentence whole, and the dropzones rise 79px. That line was then rewritten six times, over what to call the thing under the pointer and then over how to say any of it aloud, which is the part worth recording. *Phrase* is the grammarian's word, *unit* is the machinery's, and *word* is the one that undersells it, since the gloss takes a content word together with whatever attaches to it and translating word by word is exactly what it does not do. **Expression** is the everyday word for a logical group of words and carries no definition with it. What was still wrong at that point was the grammar rather than the noun: *A finished book, open anywhere, no connection needed. Expressions in French translate under the pointer* is three fragments stacked and then a sentence in which the expressions do the translating. It reads **A finished book you can open anywhere, even offline. Point at any French expression to see what it means**, which is two ordinary sentences with a reader in them. The lesson worth keeping is the first one: shortening a sentence and removing a sentence are different tools, and a reader who has to work out what a line means has not been given less to read. |
| What moves on the front door while the engine is still coming? | **Nothing, and the line says what is happening.** A sweeping rule ran under *Warming up the workshop…* — the builder's one device for a wait with no reading on it, and the wrong device here: it is the first screen a reader ever meets, the motion is the only thing moving on it, and it draws the eye to a wait they can neither shorten nor act on. It also made the foot two lines tall while it ran, so the *Next* button beside it stepped down 8px the moment the engine landed. The sentence stays; the rule is gone. The device is untouched everywhere it is right, which is a wait a reader has **asked for** — a search out on Wikisource, a model thinking, a book being fetched. |
| When does the headline dock? | **On the shelf alone, and once docked it stays.** Docking lifts everything under it by 122px, which is room the shelf's cards need and nothing the two file routes ever wanted. Hung on the first press *of any route*, it took the tab strip out from under the finger that had just pressed it — and only the first time, which is the shape of a glitch rather than of a page settling. Now `align` and `translate` move nothing at all: measured across nine widths from 1000 to 1920 and five heights, the only thing that changes between them is the dropzone's own width, one slot against two. The shelf still docks, on the same press that redraws the page with cards, so the motion is the one already being asked for. Coming back from the shelf does **not** undock, because dropping the page 122px the other way is the same fault facing the other direction. Dropping a file no longer docks either, for the same reason and one more: the lift would arrive exactly as the reader is watching the card they have just filled. |
| What does a finished book do when it is finished? | **It opens, and the file is the second thing offered.** The done screen ended in `Download`, so the one route that ends in a book ended in a file being filed: a reader who had waited out a build was handed a 400 KB download and left to find it. `Read it now` is the solid button and the download drops to a line under it, which is the ordering every other screen here already has — a shelf card opens the book and hides building it in the drawer. Nothing is lost by reading first, because the book is kept on the reader's own row in this browser as it is made, so the way back to it is the front door rather than a folder. Truly **automatic** was declined and is not a matter of taste: a build ends minutes after the press that started it, and a browser blocks a tab opened outside a click, so an auto-open would be silently swallowed on the setups that block it and would be worse than not offering one. The tab is opened with the book **already in it** — `S.html` is in hand, nothing is awaited — where the shelf's own row must open a blank tab first and fill it from storage. Two things a reader is not told and does not need to be: the book at that address is not a file, so it cannot be bookmarked and is gone when the tab closes, which is what the download is still for; and its bookmark and theme are kept against the builder's own site rather than the file's, so reopening from the front door resumes and opening the downloaded copy starts at page one. **The book is written into the tab, never addressed**, which is the second half of this and was found by a reader pressing it: handed a `blob:` URL, Safari gives the tab to its downloader instead of drawing it, so the one press that exists to spare somebody a file put a second file in their Downloads. Writing it in involves no URL at all, so there is nothing for a downloader to take, and the tab still keeps this site's own storage, which is where the bookmark and the theme are. The reader's own row had the same fault and nobody had met it, because only the done screen had been pressed; both go through one path now. Neither engine here reproduces it — Chromium and WebKit both draw a blob tab, and the policy is Safari's own rather than the engine's — so what the test asserts is the book drawn **and nothing downloaded**. Asserting the *address* is exactly what let it through: `blob:` is what a rendered tab and a downloaded file both report. |
| Why is the right of the reader's header empty? | **Because eight groups of controls needed a wider window than anybody has, and the header carries the book now while the desk carries the tools.** The row measured 1031px against a 371px title, so it wanted **1490px** before it fitted on one line: below that it broke in two, and the controls, being one block on a line of their own, fell flush left. That left an L of nothing — up to 369px at the end of the row and the whole top right corner, 1009px of it at 1440. Four ways of *arranging* that row were drawn on the real book and all four were wrong in the same way, which is the finding worth keeping: **spreading a toolbar is not a fix**, because closeness is what says which controls belong together, so justifying the row only moved the hole inside it, and centring it made every control's position depend on what else was in the row — and this reader genuinely varies, since the hover pill is absent on a fully glossed book, the published toggle on a translated one, the download until a format is built, and the edits link until you correct something, so Chapters would stand 75px apart on Candide and Micromégas and would move under the reader's hand the first time they fixed a phrase. Consolidating instead (`Aa` for `A− A+`, the link inside the download menu, the ⓘ folded into its toggle) was drawn too, and it holds only at the top of the range: even at 926px the row wants 1377px for Candide, 1498 for 80 Days, and **+123px in German**, so two of seven shelf books truncate at 1440 and none survives a 1280 window. What ships instead moves **chapters, bookmarks, text size and the hand-off pair to a bar at the foot of the desk**, which is the reasoning that already put day-and-night and the way back to the builder down there: they are about the act of reading, where the header's own controls are about the text. The header comes to **72px at every width from 900 up**, one line, the reading controls holding the right margin, and it is the book's **name** that gives way now (a flex basis of 0 asks for nothing and takes only what is left), with the masthead standing down under 900px because where only one fits, the book wins. Two costs, both accepted: the desk gives 66px to the bar, so the book gains 4px rather than the 122 that hiding the chrome would give; and the panels that used to drop from the header now rise from the bar, **anchored over the control that opened them** rather than centred, because centred put all three on the gutter and hid the same lines of both pages. The bar's height is **measured** into `--deskbar-h` rather than assumed, since it wraps to two rows on a phone and a constant would have the book running underneath it. |
| May a filter hide a book over how much work has been done to it? | **No, and *Read through* was the last one doing it.** It went for the reason *Nothing abridged* went before it, one step further out: abridgement is at least a fact about the edition, where whether anybody here has read a book end to end is a fact about **this shelf's own work** and nothing about the novel. So a reader pressing it was shown four books and given no account of where the other three had gone — and the three that vanished are neither worse books nor worse builds, only ones nobody has sat down with yet. What the chip stood for is already on the cards, in the two forms it honestly takes: a measured coverage on the ones that have been read, and a sentence in the book's own note on the ones that have not. Two chips are left, and both are facts about the book in the reader's hands: whether there is more than one English to choose from, and whether it builds inside ten minutes. Nothing else changes — `read_through` still sets the card's unread styling and still decides which of those two things a card says, so the shelf keeps saying it and stops sorting by it. |
...
- **A second-library card still spells its title out of the URL.** `_titlecase`
  reads the slug, so a reader is offered *The Devils Pool* and *Salammbo* where
  the site itself writes *The Devil's Pool* and *Salammbô* — a URL cannot hold an
  apostrophe or an accent, and the slug is that mangling rather than the title.
  Harmless until now and cheap to fix from here: reading the row instead of the
  href puts the site's own `<span property="schema:name">` in hand at no extra
  request. Left alone deliberately, because the same argument reaches the
  **author**, and there the codebase has already answered it the other way —
  `_fold` exists precisely to compare `emile-zola` with `Émile Zola`, and undoing
  that is a bigger change than a nicer card.
