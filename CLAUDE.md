...
- **A reader who brings a scan is not told so, in the browser.** `looks_scanned`
  weighs bytes of file against characters of text and is wired into the CLI only,
  so the web builder takes a photograph of a book without a word: a reader
  brought the Internet Archive Nausea at **80.6 bytes per character** and paid to
  align it, when the same translation sat in their downloads digitally typeset at
  1.1 and every joined word in it (`isvery`, `itsent`, `firstsheet`) is correct
  there. 1.9% of that build's English words appear nowhere in the clean edition,
  and 835 of them are two real words run together. The measure exists, costs
  nothing, and needs only to reach the price screen, before anything is paid for.
