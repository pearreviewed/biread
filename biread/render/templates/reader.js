(function () {
  ...
  // The working builder, wired only in the hosted edition. A shared book ships
  // #builder-flow inert and never runs a line of this — so a travelling file has
  // no file drop and, later, no key field. Phase 1: drop a French file, preview
  // it, choose how the English comes. The on-your-key build step lands next.
  if (DATA.builder) initBuilder();
  function initBuilder() {
    ...
    document.getElementById('builder-placeholder').hidden = true;
    document.getElementById('builder-flow').hidden = false;
    var zone = document.getElementById('builder-dropzone');
    var input = document.getElementById('builder-file');
    var loaded = document.getElementById('builder-loaded');

    function accept(file) {
      if (!file) return;
      var reader = new FileReader();
      reader.onload = function () { showLoaded(file.name, String(reader.result || '')); };
    ...
    // A quick, honest preview: blank-line paragraph blocks and a word count. The
    // authoritative chapter/paragraph split is cleanup.py's, run at build time.
    function showLoaded(name, text) {
      ...
      var row = mk('div', 'builder-file-row');
      row.appendChild(mk('span', 'builder-file-name', name));
      var change = mk('button', 'builder-file-change', 'change');
      change.type = 'button';
      change.addEventListener('click', function () {
        loaded.hidden = true; zone.hidden = false; input.value = '';
      });
      row.appendChild(change);
      loaded.appendChild(row);

      loaded.appendChild(mk('div', 'builder-stats',
        '~' + paragraphs.toLocaleString() + ' paragraphs · ~' + words.toLocaleString() + ' words'));
      loaded.appendChild(mk('div', 'builder-stats-note',
        'A quick look — the chapters and paragraphs are set exactly when you build.'));

      loaded.appendChild(mk('div', 'builder-q', 'How should the English come?'));
      var paths = mk('div', 'builder-paths');
      paths.appendChild(pathCard('generate', 'Generate a translation',
        'A fresh AI translation, on your own key. Costs a few dollars.'));
      paths.appendChild(pathCard('published', 'I have a published one',
        'Bring an existing translation — aligned by meaning. Free.'));
      loaded.appendChild(paths);
      loaded.appendChild(mk('div', 'builder-next', ''));
    }

    function pathCard(path, title, sub) {
      var card = mk('button', 'builder-path');
      card.type = 'button';
      card.setAttribute('data-path', path);
      card.appendChild(mk('div', 'builder-path-title', title));
      card.appendChild(mk('div', 'builder-path-sub', sub));
      card.addEventListener('click', function () {
        var all = loaded.querySelectorAll('.builder-path');
        for (var i = 0; i < all.length; i++) all[i].classList.remove('selected');
        card.classList.add('selected');
        loaded.querySelector('.builder-next').textContent =
          'Next — choose a model and add your key. That step lands soon.';
      });
      return card;
    }
  }
